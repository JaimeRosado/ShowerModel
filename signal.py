...
Signal object.
    ...
    #In addition, the very begninning of the shower profile is ignored to speed up calculations
    ...
    #Only discretization points within the telescope field of view contributes to the signal
    collection = telescope.area * np.cos(theta) / 4000000. / np.pi / distance**2
    ...
    collection_cher = collection * 2. / np.sin(beta) * (
                 a / theta_c * np.exp( -beta / theta_c ) + b / theta_cc * np.exp( -beta / theta_cc ) )
    ...
    if atm_trans:
        ...
        thickness = np.array( atmosphere.h_to_Xv( atmosphere.h0 + telescope.z ) - X_vert )
        thickness[thickness!=0] = thickness[thickness!=0] / np.sin( alt[thickness!=0] )
        thickness[thickness==0] = 100000. * distance[thickness==0] * rho[thickness==0]
        trans = np.exp( -thickness / 1645. ) #Only points within the telescope FoV, otherwise trans=0
    ...
    signal['Npe_cher'] = cherenkov.N_ph.loc[points] * collection_cher * wvl_factor
...
def image(signal):
    ...
    N_pix_d = math.ceil(math.sqrt(2. * telescope.N_pix)) #Number of pixels across a diameter
