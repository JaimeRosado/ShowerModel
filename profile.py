...
#### Constructor ###################################
def Profile(E=_E, theta=_theta, alt=None, prf_model=_prf_model, X_max=None, X0_GH=None, lambda_GH=None,
...
Shower : Contructor of Shower object.
    ...
    #The columns of the output DataFrame are: the slant depth in g/cm^2, the shower age s,
    #the energy deposit E_dep in MeV, and the number N_ch of charged particles with energy above 1MeV
    profile = _Profile( columns=['X', 's', 'dX', 'E_dep', 'N_ch'] )
    profile.atmosphere = atmosphere

    #The input shower parameters along with some geometric parameters are included as atributes of the DataFrame
    profile.E = E
    if alt==None:
        alt = 90. - theta
    else:
    ...
    if isinstance(prf_model, pd.DataFrame): #DataFrame containing dE/dX at steps in X
        prf_model.sort_index(axis=0, ascending=True, inplace=True) #Sorted to allow for interpolation
        X_model = np.array(prf_model.iloc[:,0]) #The first column must be X in g/cm2
        dE_dX_model = np.array(prf_model.iloc[:,1]) #The second column must be dE_dX in MeV.cm2/g
        ...
        if X_max==None:
            ...
            print('Warning: The input X_max does not match the maximum of the input energy deposit profile.')
        ...
        if E<profile.E_dep.sum():
            profile.E = E_dep.sum()
            raise ValueError('The input shower energy is lower than the integraged energy deposit profile.')
        elif E>profile.E_dep.sum()*1.2:
            print('Warning: The input energy is greater than the integrated energy deposit profile by more than 20\%.')
        
    elif prf_model=='Greisen':
        if X_max==None:
            X_max = 36.7 * np.log( E / 81. )   #E_c=81 MeV is the critical energy in air
        profile.X_max = X_max
        s = 3. * X / ( X + 2. * X_max )  #Shower age
        profile.s = s
        ...
        #where t=X/lambda_r , lambda_r = 36.7 g/cm^2 is the radiation length in air
        N_ch[s>0] = 0.31 / np.sqrt(X_max/36.7) * np.exp( X[s>0] /36.7 * ( 1. - 1.5 * np.log(s[s>0]) ) )
        #s>0 prevents from errors for discretization steps with X=0, where the atmosphere is undefined 
        ...
        #where  alpha(s) is the mean ionization energy loss per electron for an energy cut of 1MeV
        profile.E_dep = profile._alpha(profile.s) * profile.N_ch * profile.dX
        
    elif prf_model=='Gaisser-Hillas':
        if X_max==None: #If not given, a typical value according to Heitler model is used
        ...
        if (X0_GH==None) or (lambda_GH==None): #If not given, typical values according to Auger data are used
            x = 6. + np.log10(E)
            R = 0.26 - 0.04 * (x - 17.9)
            L = 226.2 + 7.1 * (x - 17.9)
            if X0_GH==None:
        ...
        #Gaisser-Hillas profile: N_ch = N_ch_max * [ (X-X0) / (X_max-X0) ]**( (X_max-X0) / lambda ) * exp[ -(X-X_max) / lambda ]
        X_min = max(X0_GH, 0.) #X0_GH is expected to be negative
        N_ch[X>X_min] = ( (X[X>X_min]-X0_GH) / (X_max-X0_GH) )**( (X_max-X0_GH) / lambda_GH ) * np.exp( -(X[X>X_min]-X_max) / lambda_GH )
        #X>X_min prevents from errors for discretization steps with X=0, where the atmosphere is undefined
...
Attributes
----------
atmosphere : Atmosphere discretization used to construct the shower profile.
...
show : Show the shower profile, both number of charged particles and energy deposit, as a function of slant depth.
...
Shower : Constructor of Shower object.
    ...
    def _Greisen_norm(self):
        """Calculate the normalization constant K that relates a Greisen profile to the actual shower size N(s).
        """
        return _Greisen_norm(self.E, self.X_max)
    
    
    def _GH_norm(self):
        """Calculate the normalization constant K that relates a Gaisser-Hillas profile to the actual shower size N(s).
        """
        return _GH_norm(self.E, self.X_max, self.X0_GH, self.lambda_GH)
    
    
    def _alpha(self, s):
        """Calculate the mean ionization loss rate per electron in MeV/g.cm^2 as a function of shower age.
        """
        return _alpha(s)
...
def _Greisen_norm(E, X_max):
    """Calculate the normalization constant K that relates a Greisen profile to the actual shower size N(s)
for an energy cut of 1MeV, that is, N(s) = K * N_g(s), with the following constraint:
E = int[ dE/dX * dX ] = int[ alpha(s) * N(s) * dX/ds *ds ].
    ...
    #Greisen profile
    #N_g = 0.31/sqrt(X_max/lambda_r) * exp[ X/lambda_r * (1-3/2*log(s)) ]  ,   lambda_r=36.7g/cm^2
...
def _GH_norm(E, X_max, X0_GH, lambda_GH):
    """Calculate the normalization constant K that relates a Gaisser-Hillas profile to the actual shower size N(s)
for an energy cut of 1MeV, that is, N(s) = K * N_GH(s), with the following constraint:
E = int[ dE/dX * dX ] = int[ alpha(s) * N(s) * dX/ds *ds ].
    """
    s = np.arange(0., 3., 0.01) + 0.005   #The shower age s ranges from 0 to 3
    X = 2. * X_max * s / (3. - s)             #slant depth
    ...
    #Gaisser-Hillas profile: N_ch = N_ch_max * [ (X-X0) / (X_max-X0) ]**( (X_max-X0) / lambda ) * exp[ -(X-X_max) / lambda ]
    N_GH = ( (X-X0_GH) / (X_max-X0_GH) )**( (X_max-X0_GH) / lambda_GH ) * np.exp( -(X-X_max) / lambda_GH )
...
def _alpha(s):
    """Calculate the mean ionization loss rate per electron in MeV/g.cm^2 as a function of shower age such that:
dE/dX = alpha(s) * N(s)
