...
#### Constructor ###############################
def Signal(telescope, shower, projection=None, atm_trans=True, tel_eff=True, **kwargs):
...
atm_trans : True if the atmospheric transmision is included to transport photons.
tel_eff : True if the telescope efficiency is included to calculate the signal.
...
**kwargs {wvl_ini, wvl_fin, wvl_step}: Options to modify the wavelenght interval when tel_eff==False.
    If None, the wavelength interval defined in the telescope is used.
...
Signal object.
    ...
    #This function is normally called from Event. If not, projection must be generated.
    ...
    distance = np.array(projection.distance)
    theta = np.array(projection.theta)
    alt = np.array(projection.alt)
    FoV = np.array(projection.FoV)
    FoV[signal.profile.s<0.01]=False #The very beginning of the shower is ignored to speed up calculations
    
    #Solid angle fraction covered by the telescope area
    #Only discretization points within the telescope field of view contributes to the signal
    collection = np.zeros_like(distance)
    collection[FoV] = telescope.area * np.cos( np.radians(theta[FoV]) ) / 4000000. / np.pi / distance[FoV]**2
    
    #Collection efficiency for the angular distribution of Cherenkov light
    #See F. Nerling et al., Astropart. Phys. 24(2006)241.
    beta = np.radians(projection.beta)
    theta_c = np.radians(cherenkov.theta_c)
    theta_cc = np.radians(cherenkov.theta_cc)
    collection_cher = collection * 2. / np.sin(beta) * (
                 cherenkov.a / theta_c * np.exp( -beta / theta_c ) + cherenkov.b / theta_cc * np.exp( -beta / theta_cc ) )

    #Relative fluorescence contribution from each shower point at each band (between wvl_ini and wvl_fin)
    #The atmospheric transmission is included later
    rel_fluo = fluorescence.copy()
    if tel_eff:
        rel_fluo *= eff_fluo #34 bands
    rel_fluo = rel_fluo.loc[:,wvl_ini:wvl_fin] #selection of bands within the wavelenght range
    
    if atm_trans:
        #Atmospheric transmission at 350 nm. Only Rayleigh scattering is considered
        thickness = np.array( atmosphere.h_to_Xv( atmosphere.h0 + telescope.z ) - atmosphere.X_vert )
        thickness[thickness!=0] = thickness[thickness!=0] / np.sin( np.radians(alt[thickness!=0]) )
        thickness[thickness==0] = 100000. * distance[thickness==0] * atmosphere.rho[thickness==0]
        trans = np.zeros_like(thickness)
        trans[FoV] = np.exp( -thickness[FoV] / 1645. ) #Only points within the telescope FoV, otherwise trans=0
    
        #Relative fluorescence contribution including atmospheric transmission
        for wvl in rel_fluo:
            rel_fluo[wvl] *= trans ** ( ( 350. / wvl )**4 )
        
        #Wavelenght factor for Cherenkov contribution to signal from each shower point
        wvl_factor = pd.DataFrame()
        for wvl in wvl_cher:
            wvl_factor[wvl] = trans ** ( ( 350. / wvl )**4 ) / wvl**2 #wvl**2 -> (wvl**2 - wvl_step**2 / 4.)
        if tel_eff:
            wvl_factor *= eff_cher
        wvl_factor = wvl_factor.sum(axis=1) * wvl_step / ( 1. / 290. - 1. / 430. )
        
    elif tel_eff: #If atmospheric transmission is not included
        #The wavelength factor of Cherenkov signal is the same for all shower points
        wvl_factor = eff_cher / wvl_cher**2 #wvl_cher**2 -> (wvl_cher**2 - wvl_step**2 / 4.)
        wvl_factor = wvl_factor.sum() * wvl_step / ( 1. / 290. - 1. / 430. )
        
    else: #If neither the atmospheric transmission nor the telescope efficiency are included
        #The wavelength factor of Cherenkov signal only depends on the integration wavelength interval
        wvl_factor = ( 1. / wvl_ini - 1. / wvl_fin ) / ( 1. / 290. - 1. / 430. )
        
    #Number of photoelectrons due to fluorescence light emitted from each shower point
    ...
    #Number of photoelectrons due to Cherenkov light emitted from each shower point
    signal['Npe_cher'] = cherenkov.N_ph * collection_cher * wvl_factor
    #Number of photoelectrons due to Cherenkov light within the FoV
    signal.Npe_cher_sum = signal.Npe_cher.sum()
    #Total number of photoelectrons from both light components emitted at each shower point
...
class _Signal(pd.DataFrame):
    """DataFrame containing the signal produced by a shower detected by a telescope.
    
Columns
-------
Npe_cher : Number of photoelectrons per discretization step due to Cherenkov light.
Npe_fluo : Number of photoelectrons per discretization step due to fluorescence light.
...
Methods
-------
show_projection : Show the projection of the shower track viewed by the telescope in both zenith and camera projections.
...
show : Show the signal evolution as a function of both time and beta angle (relative to the shower axis direction).
    """
    def show_projection(self, shower_size=True, axes=True, max_theta=30., X_mark='X_max'):
        """Show the projection of the shower track viewed by the telescope in both horizonta and FoV coordinates systems.
...
def _show(signal):
    ...
    #Arrival time interval in microseconds (or nanoseconds) for each discretization step.    
    ...
    #Number of photoelectrons due to each light component (and total) per unit time
    ...
    #Number of photoelectrons due to each light componente (and total) per unit of beta angle
...
def image(signal):
    ...
    points = np.array(signal.Npe_total>0.)
    
    for point in signal.projection:
        pass
