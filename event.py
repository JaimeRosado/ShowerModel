...
atm_trans : Bool indicating whether the atmospheric transmision is included to transport photons.
...
Event object.
    ...
    for telescope in observatory:
        ...
        signal = tm.Signal(telescope, shower, projection, atm_trans, tel_eff, **kwargs)
...
#### Class #########################
class _Event():
    """Event object containing the characteristics of a shower, an observatory and the signal produced by the shower in each telescope.
...
event_type : Name of subclass of Event.
    Presently, only the parent class Event and the subclass GridEvent are available. More subclasses to be implemented.
shower : Shower object.
track : Track object.
...
Methods
-------
show_projection : Show the projection of the shower track viewed by a telescope in both zenith and camera projections.
...
show_signal : Show the signal evolution as a function of both time and beta angle (relative to the shower axis direction)
    for a chosen telescope of the observatory.
show_geometry2D : Show the shower track together with the telescope positions in a 2D plot.
show_geometry3D : Show the shower track together with the telescope positions in a 3D plot.
    """
    event_type = None

    def show_projection(self, tel_index=0, shower_size=True, axes=True, max_theta=30., X_mark='X_max'):
...
(ax1, ax2) : PolarAxesSubpot objects.
        ...
        return show_projection(projection, profile, shower_size, axes, max_theta, X_mark)
...
(ax1, ax2) : AxesSubplot objects.
    ...
    def append_telescope(self, telescope):
        """Append a telescope to the observatory and generate the corresponding prjection and signal."""
        self.observatory.append(telescope)
        self.observatory.N_tel+=1
        projection = telescope.Projection(self.shower)
        self.projections.append(projection)
        self.signals.append(telescope.Signal(self.shower, projection, self.atm_trans))

        
    def show_geometry2D(self, x_min=-1., x_max=1., y_min=-1, y_max=1.,
                        X_mark='X_max', shower_size=True, signal_size=True, tel_index=False):
...
X_mark : Reference slant depth in g/cm^2 of the shower track to be marked in the figure, default to X_max.
    If X_mark is set to None, no mark is included.
shower_size : Bool indicating whether the radii of the shower track points are proportional to the shower size.
signal_size : Bool indicating whether the radii of the telescope position points are proportional to the signal.
...
AxesSubplot object.
        ...
        if X_mark=='X_max':
            X_mark = self.shower.X_max
        observatory = self.grid if self.event_type=='GridEvent' else self.observatory
        from .tools import show_geometry
        return show_geometry(self, observatory, '2d', x_min, x_max, y_min, y_max,
                             X_mark, shower_size, signal_size, tel_index, False, False)
       
        
    def show_geometry3D(self, x_min=-1., x_max=1., y_min=-1, y_max=1.,
                        X_mark='X_max', shower_size=True, signal_size=True, xy_proj=True, pointing=False):
...
Axes3DSubplot object.
        ...
        return show_geometry(self, observatory, '3d', x_min, x_max, y_min, y_max,
                             X_mark, shower_size, signal_size, False, xy_proj, pointing)
        
        
    def show_distribution(self, grid=None, size_x=2., size_y=2., N_x=10, N_y=10, atm_trans=None, tel_eff=None, **kwargs):
...
grid : Existing Grid object to be used.
    If None, a new Grid object is generated from the specificed dimensions and the characteristics of
    the telescope with tel_index=0 of the observatory. If given, size_x, size_y, N_x, N_y are not used.
...
atm_trans : Bool indicating whether the atmospheric transmision is included to tranport photons.
    If None, this option is set to be the same as the original Event object.
tel_eff : Bool indicating whether the telescope efficiency is included to calculate the signals.
    If None, this option is set to be the same as the original Event object.
**kwargs {wvl_ini, wvl_fin, wvl_step}: Options to modify the wavelenght interval when tel_eff==False.
    If None, the wavelength interval of the grid telescopes is used.
...
(ax1, ax2, cbar) : AxesSubplot objects and Colorbar object (if 2D grid).
        ...
        atm_trans = self.atm_trans if atm_trans==None else atm_trans
        tel_eff = self.tel_eff if tel_eff==None else tel_eff
...
class _GridEvent(_Event):
    ...
    def show_distribution(self): #Overwrite the method of the parent class
        """Show the distribution of photons (or photoelectrons) per m$^2$ in an either 1D or 2D plot, depending on the grid dimensions."""
...
def _show_distribution(grid_event):
    ...
    area = grid.tel_area #Detection area of the telescope, default to grid cell area.    
    
    signal_cher = np.array( [signal.Npe_cher_sum for signal in grid_event.signals] ) / area
    signal_fluo = np.array( [signal.Npe_fluo_sum for signal in grid_event.signals] ) / area
    ...
    if (N_x>1) and (N_y>1):    #2D grid
        ...
        ax1.imshow(signal_cher, norm=mpl.colors.LogNorm(vmin=signal_min, vmax=signal_max), extent=extent) #cmap=viridis
        psm = ax2.imshow(signal_fluo, norm=mpl.colors.LogNorm(vmin=signal_min, vmax=signal_max), extent=extent) 
