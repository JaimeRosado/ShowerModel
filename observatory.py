...
def Array25(telescope=None, tel_type='MST', x_c=_x, y_c=_y, z_c=_z, theta=None, alt=None, az=None,
                 R=0.341, rot_angle=0.):
    """Make an array of 25 telescopes based on a layout of MST telescopes of CTA. The pointing directions
of all the telescopes are set equally, but they can be modified individually (along with other porperties) later on.
...
tel_type : Subclass of Telescope to be used when telescope==None.
    Default to MST with field of view of 8 degrees, mirror area of 113 m$\,^2$ and 1800 pixels.
...
az : Azimuth angle (from north, clockwise) in degrees of the telescope pointing directions.
R : Radius in km of the array.
rot_angle : Rotation angle in degrees of the array (clockwise).
...
Observatory object.
    ...
    if isinstance(telescope, _Telescope):
        ...
        if az==None:
            az = telescope.az
        telescope = telescope.copy(x=x_c, y=y_c, z=z_c, theta=theta, alt=alt, az=az) #tel_type is ignored
    elif telescope==None:
        ...
        if az==None:
            az = _az
        telescope = tm.Telescope(x_c, y_c, z_c, theta=theta, alt=alt, az=az, tel_type=tel_type)
    else:
...
def Grid(telescope=None, tel_type='GridElement', x_c=_x, y_c=_y, z_c=_z, theta=None, alt=None, az=None,
              size_x=2., size_y=2., N_x=10, N_y=10):
    """Make a rectangular grid of telescopes across the x and y directions. The pointing directions
of all the telescopes are set equally, but they can be modified individually (along with other porperties) later on.
    
The telescope index is sorted first by y (from max to min) and then by x (from min to max),
so that tel_index=0 corresponds to the telescope placed at the corner with minimum x and maximum y.
...
Observatory object.
    ...
    if isinstance(telescope, _Telescope):
        ...
        if az==None:
            az = telescope.az
        telescope = telescope.copy(z=z_c, theta=theta, alt=alt, az=az) #tel_type is ignored
    elif telescope==None:
        if tel_type=='GridElement':
            #The FoV is 180 degrees around zenith direction and area equal to one grid cell
            theta = 0.
            alt = None
            az = 0.
            telescope = tm.Telescope(z=z_c, theta=theta, alt=None, az=0., tel_type=tel_type, area=cell_area)
        else:
            #Default theta and az values are those of Telescope
            if theta==None:
                theta = _theta
            if az==None:
                az = _az
            telescope = tm.Telescope(z=z_c, theta=theta, alt=None, az=az, tel_type=tel_type)
    else:
        raise ValueError('The input telescope is not valid.')
...
class _Observatory(list):
    """List containing the characterestics of an observatory and its constituent telescopes.
    
Attributes
----------
obs_type : Name of the subclass of Observatory.
    Presently only the parent class Observatory and the Array25 and Grid subclasses are available. More subclasses to be implemented.
N_tel : Number of telescopes.
x_c : East coordinate in km of the center of the grid.
y_c : North coordinate in km of the center of the grid.
z_c : Height of the grid in km above ground level.
size_x : Size of the observatory in km across the x direction.
size_y : Size of the observatory in km across the y direction.
R : Radius of the observatory in km (only Array25 objects).
rot_angle : Rotation angle in degrees (clock-wise) of the array (only Array25 objects).
N_x : Number of cells across the x direction (only Grid objects).
N_y : Number of cells across the y direction (only Grid objects).
cell_area : Area in m$\,^2$ of one cell (only Grid objects).
tel_type : Name of the subclass of Telescope.
tel_apert : Angular diameter in degrees of the telescope FoV.
tel_area : Detection area of each telescope in m$\,^2$ (e.g., mirror area of an IACT).
tel_N_pix : Number of camera pixels.

Note: Attributes inherited from Telescope (i.e., tel_type, tel_apert, tel_area and tel_N_pix) are not updated when
telescopes are modified or appended.

Methods
-------
show : Show the telescope positions and indexes of the observatory in a 2D plot.
    """
    obs_type = None
