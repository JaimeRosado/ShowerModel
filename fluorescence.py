...
using the parameterization described in D. Morcuende et al., Astropart. Phys. 107(2019)26 and references therein.
...
Fluorescence object
    ...
    P0 = 800.
    T0 = 293.
    #Y0_337=7.05 ph/MeV is the fluorescence yield of the band at 337nm in dry air at reference conditions
    ...
    for i, (wvl, I_rel, PP0, PPw, a) in enumerate(model):
        if PPw==0:  #For some bands, no information on the quenching contribution from water vapor is available
            fluorescence[wvl] = N0_337 * I_rel * ( 1. + P0 / PP0 ) / ( 1. + profile.atmosphere.P / PP0 * ( T0 / profile.atmosphere.temp )**( 0.5 - a) )
        else:
            fluorescence[wvl] = N0_337 * I_rel * ( 1. + P0 / PP0 / ( 1. + ( ( profile.atmosphere.P - profile.atmosphere.P_w ) / PP0 + profile.atmosphere.P_w / PPw )
...
class _Fluorescence(pd.DataFrame):
    """DataFrame containing the fluorescence light production at each of the 34 bands
of the fluorescence spectrum in the 290 - 430 nm range based on the parameterization described in
...
Methods
-------
show : Show the production of fluorescence photons in the 290 - 430 nm range as a function of slant depth.

    """
    def show(self):
        """Show the production of fluorescence photons in the 290 - 430 nm range as a function of slant depth.
...
AxesSubplot object.
        ...
        ax.axes.yaxis.set_label_text("Fluorescence production (Photons·cm$\,^2$/g)")
        return ax
