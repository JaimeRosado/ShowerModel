...
Cherenkov object.
    ...
    cherenkov.N_ph = 2. * np.pi / 137.036 * ( 1. / 290. - 1. / 430. ) * 1e12 * C_E * profile.N_ch * profile.dl
...
class _Cherenkov(pd.DataFrame):
    """DataFrame containing the Cherenkov ligth production in the 290 - 430 nm range
as well as the parameters determining the angular distribution of Cherenkov emission
based on the parameterization described in F. Nerling et al., Astropart. Phys. 24(2006)241.
...
theta_cc : Second parameter (degrees) of the angular distribution of Cherenkov emission.
b : Third parameter of the angular distribution of Cherenkov emission.
theta_cc : Fourth parameter (degrees) of the angular distribution of Cherenkov emission.
...
Methods
-------
show : Show the production of Cherenkov photons in the 290 - 430 nm range as a function of slant depth.
    """
    def _E_factor(self, s, Eth, Emax):
        """Calculate a factor enclosing the dependence of the Cherenkov production on
both the Cherenkov energy threshold E_th and the normalized energy distribution of electrons.
        """
...
AxesSubplot object.
        ...
        ax.axes.yaxis.set_label_text("Cherenkov production (photons·cm$\,^2$/g)")
        return ax
...
def _E_factor(s, Eth, Emax):
    """Calculate a factor enclosing the dependence of the Cherenkov production on
both the Cherenkov energy threshold E_th and the normalized energy distribution of electrons,
as they vary with the shower age:

E_factor = (mc^2)^2 * int( ( 1/Eth^2 - 1/E^2 ) * f(E) * d(ln(E)) )
where  f(E,s) = 1/N(s) * dN(E,s)/d(ln(E)) ,  with int( f(E,s)*d(ln(E)) ) = 1
See F. Nerling et al. Astropart. Phys. 24(2006)241.
    """
    #Parameterization of the energy distribution of secondary electrons as a function of the shower age
    #f(E,s)= a0(s) * E / [E+a1(s)] / [E+a2(s)]^s
    s = np.array(s)
    Eth = np.array(Eth)
    ...
    for i in range(0, len(factor)):
        if Eth[i]>Emax:
            ...
            factor[i] = a0[i] * ( tail(Eth[i], Eth[i], s[i]) - tail(Eth[i], Emax, s[i]) )
        else:        # Eth < Etail <= Emax
            lnE = np.arange(np.log(Eth[i]), 8., 0.1) + 0.05
            E = np.exp(lnE)
            #Normalized electron energy distribution: f(E,s) = 1/N(s) * dN(E,s)/d(ln(E)) ,  with int( f(E,s)*d(ln(E)) ) = 1
            f = a0[i] * E / ( E + a1[i] ) / ( E + a2[i] )**s[i]
            factor[i] = np.sum( ( 1. / Eth[i]**2 - 1. / E**2 ) * f ) * 0.1
            #factor[i]=factor[i]+a0[i]/Etail**s[i]*(1./s[i]/Eth[i]**2-1./(s[i]+2.)/Etail**2)
            if Etail<Emax:
                factor[i] = factor[i] + a0[i] * ( tail(Eth[i], Etail, s[i]) - tail(Eth[i], Emax, s[i]) )
