...
#### Constructor ###############################
def Telescope(x=_x, y=_y, z=_z, theta=_theta, alt=None, az=_az, tel_type=_tel_type,
              efficiency=None, apert=None, area=None, N_pix=None):
...
tel_type : Subclass of Telescope to be used, default to MST.
    If None, the parent class Telescope is used.
    Presently only the MST and GridElement subclasses are available. More subclasses to be implemented.
efficiency : DataFrame containing detection efficieny data.
    If None, the default efficiency of the selected tel_type.
    If given, it must have two columns with wavelength in nm (with constant discretization step) and efficiency (decimal fraction).
...
Array25 : Make an array of 25 telescopes based on a layout of CTA.
    ...
    if tel_type=='MST':
        ...
        telescope.tel_type = tel_type  #If tel_type!=None -> new telescope type
    ...
    #Coordinates of a point at 1km distance from the telescope in the north direction
    x_north, y_north, z_north = telescope.hor_to_FoV(0., 1., 0.)
    #Position angle of the the right-hand direction relative to north direction 
    phi_right = - telescope.xy_to_phi(x_north, y_north) #(-270, 90]
    telescope.phi_right = phi_right + 360. if phi_right<0. else phi_right #[0, 360)
    
    if (apert!=None) or (N_pix!=None):
        if apert!=None:
            telescope.apert = apert
            telescope.sol_angle = 2. * math.pi*( 1. - math.cos( math.radians(telescope.apert) / 2. ) ) #str

        if N_pix!=None:
            telescope.N_pix = N_pix

        telescope.sol_angle_pix = telescope.sol_angle / telescope.N_pix             #str
        telescope.apert_pix = 2. * np.degrees( np.arccos( 1. - telescope.sol_angle_pix / 2. / math.pi ) ) #deg
    
    if area!=None:
        telescope.area = area
        
    if isinstance(efficiency, pd.DataFrame):
        efficiency.sort_index(axis=0, ascending=True, inplace=True) #Sorted to allow for interpolation
        wvl = np.array(efficiency.iloc[:,0]) #The first column must be wavelength in nm
        telescope.wvl_cher = wvl
        telescope.wvl_ini = wvl[0]
        telescope.wvl_step = wvl[1] - wvl[0]
        telescope.wvl_fin = wvl[-1] + telescope.wvl_step/2.  #-> wvl = np.arange(wvl_ini, wvl_fin, wvl_step)
        if not np.all(np.diff(wvl)==telescope.wvl_step):
            raise ValueError("The wavelength discretization step must be constant.")
            
        eff = np.array(efficiency.iloc[:,1]) #The second column must be efficiency in decimal fraction
        if (not np.all(eff>=0.)) or (not np.all(eff<=1.)):
            raise ValueError("Efficiency must be positive and in decimal fraction.")
        telescope.eff_cher = eff
        telescope.eff_fluo = np.interp(telescope.wvl_fluo, wvl, eff, left=0., right=0.)
...
Class attributes
----------------
tel_type : Name of the subclass of Telescope.
    Presently only the parent class Telescope and the MST and GridElement subclasses are available. More subclasses to be implemented.
apert : Angular diameter in degrees of the telescope field of view.
area : Detection area in m^2 (e.g., mirror area of an IACT).
N_pix : Number of camera pixels.
...
wvl_fluo : Array containing the wavelengths of the 34 fluorescence bands included in the model.
eff_fluo : Array containing the detection efficiency at these 34 wavelenghts.
wvl_cher : Array containing the range of wavelengths in nm defined by wvl_ini, wvl_fin and wvl_step
eff_cher : Array containing the detection efficiency data in this range used to compute the Cherenkov signal.
...
alt : Altitude in degrees of the telescope pointing direction.
az : Azimuth angle (from north, clockwise) in degrees of the telescope pointing direction.
ux : x coordinate of a unit vector parallel to the telescope pointing direction.
uy : y coordinate of a unit vector parallel to the telescope pointing direction.
uz : z coordinate of a unit vector parallel to the telescope pointing direction.
sin_theta = Sine of theta (cosine of alt).
cos_theta = Cosine of theta (sine of alt).
sin_az = Sine of az.
cos_az = Cosine of az.
phi_right : Position angle phi in degrees of the right-hand direction from the telescope point of view.
...
thetaphi_to_altaz : Convert FoV coordinates theta/phi to horizontal coordinates alt/az.
...
Telescope object.
        ...
        kwargs['theta'] = kwargs.get('theta', self.theta)  #If 'alt' in kwargs and != None, theta is not used
...
Telescope.thetaphi_to_altaz : Convert FoV coordinates theta, phi to horizontal coordinates alt, az.
        """
        alt = np.radians(alt)
        az  = np.radians(az)
...
In the FoV coordinates system, x_FoV grows in the right-hand direction, y_FoV grows downward
and z_FoV grows toward the pointing direction from the telescope point of view.
...
Telescope.altaz_to_thetaphi : Convert horizontal coordinates alt, az to FoV coordinates theta, phi.
        """
        ...
        y_FoV = cos_theta * sin_az * x_hor + cos_theta * cos_az * y_hor - sin_theta * z_hor
        z_FoV = sin_theta * sin_az * x_hor + sin_theta * cos_az * y_hor + cos_theta * z_hor
...
Telescope.thetaphi_to_altaz : Convert FoV coordinates theta, phi to horizontal coordinates alt, az.
        ...
        x_hor =   cos_az * x_FoV + cos_theta * sin_az * y_FoV + sin_theta * sin_az * z_FoV
        y_hor = - sin_az * x_FoV + cos_theta * cos_az * y_FoV + sin_theta * cos_az * z_FoV
...
distance, alt, az, theta, phi : Scalars or array-like objects.
        ...
        phi = self.xy_to_phi(x_FoV, y_FoV) + self.phi_right -360. #[-90, 270) - (0, 360] = (-450, 270)
        try: #For phi being a scalar
            phi = phi + 360. if phi<0. else phi   #[0, 360)
        except: #For phi being an array 
            phi[phi<0.] = phi[phi<0.] + 360.   #[0, 360)
    ...
    def zr_to_theta(self, z, r):
        """Calculate the angle theta in degrees [0, 180] of a vector with vertical projection z and modulus r,
where theta is defined from the z axis"""
        from .tools import zr_to_theta
        return zr_to_theta(z, r)
    
    def xy_to_phi(self, x, y):
        """Calculate the angle phi in degrees [-90, 270) of the xy projection of a vector,
where phi is defined from the x axis towards the y axis (anticlockwise)"""
        from .tools import xy_to_phi
        return xy_to_phi(x, y)
    
    
    def Projection(self, track):
        """Obtain the coordinates of a shower track relative to the telescope position in both zenith and camera projection
...
Projection.show
    ...
    def show_projection(self, track, axes=True, max_theta=30., X_mark=None):
        """Obtain the polar coordinates of a shower track relative to a telescope position in both horizontal
and FoV coordinates systems and determine the fraction of the track within the telescope field of view.
In addition, show the projection of the shower track as viewed by the telescope.

Parameters
----------
track : Track object or Shower object.
axes : Bool indicating whether the axes of both coordinate systems of reference are visualized or not.
max_theta : Maximum offset angle in degrees relative to the telescope pointing direction.
X_mark : Reference slant depth in g/cm^2 of the shower track to be marked in the figure.
    If None, no mark is included.
...
Projection.show
    """
        ...
        return projection, (show_projection(projection, None, False, axes, max_theta, X_mark))

    

    
#### Subclasses ###################################
#Presently only the MST and grid_elem subclasses are available. More subclasses to be implemented.
...
class _MST(_Telescope):
    ...
    eff_fluo = np.array([0.252489433, 0.263035139, 0.287398782, 0.308913092, 0.319016638, 0.323187842, 0.326638232,
                         0.329752346, 0.339552709, 0.341029170, 0.342454324, 0.344086960, 0.345252040, 0.347248391,
                         0.347256505, 0.346686550, 0.346354164, 0.349208764, 0.349801116, 0.352296156, 0.354879676,
                         0.356753586, 0.353959270, 0.352900969, 0.352432398, 0.351494414, 0.350073889, 0.346680335,
                         0.342513534, 0.335703253, 0.330467935, 0.326751911, 0.323959630, 0.323028670])
    
    wvl_cher = np.arange(wvl_ini, wvl_fin, wvl_step) # 107 wavelengths, ending in 598nm
    #Detection efficiency taken from CTA data at these 107 wavelengths
    eff_cher = np.array([0.015491090, 0.079299180, 0.129636709, 0.172271597, 0.207375577, 0.236202184, 0.263035139,
                         0.280541802, 0.294669433, 0.305342911, 0.314872346, 0.321098876, 0.326638232, 0.331242468,
                         0.334854807, 0.337795491, 0.340290566, 0.342454324, 0.344086960, 0.345252040, 0.346172895,
                         0.346778694, 0.347248391, 0.347255182, 0.347044138, 0.346506611, 0.346354164, 0.346979403,
                         0.348025435, 0.349801116, 0.351659605, 0.353400273, 0.354879676, 0.356003591, 0.356271574,
                         0.354537850, 0.352900969, 0.351494414, 0.350073889, 0.348520911, 0.346680335, 0.344180639,
                         0.341759844, 0.339601025, 0.337372559, 0.334868408, 0.332311345, 0.329540303, 0.326751911,
                         0.323959630, 0.320980527, 0.317070704, 0.313088141, 0.308639570, 0.304189576, 0.298629946,
                         0.292593521, 0.286598981, 0.280644502, 0.275022087, 0.269524770, 0.264660241, 0.259794163,
                         0.255157961, 0.250526184, 0.246266668, 0.242256909, 0.238286004, 0.234340729, 0.230033882,
                         0.225450238, 0.219591053, 0.212757397, 0.205538060, 0.196889181, 0.185468854, 0.173996418,
                         0.164205860, 0.155335908, 0.147509786, 0.138970850, 0.132442597, 0.126951003, 0.121893936,
                         0.117054131, 0.113109258, 0.109682504, 0.106428167, 0.103273833, 0.100303912, 0.097453247,
                         0.094448853, 0.091341851, 0.088381484, 0.085533194, 0.082668619, 0.079789755, 0.077237853,
                         0.074972054, 0.072399174, 0.069519165, 0.066662492, 0.063829159, 0.061106831, 0.058511426,
                         0.057261531, 0.057550245])
    pass
...
class _GridElement(_Telescope):
    ...
    apert = 180. #deg
    #area = 100.   #m^2 It is set to one grid cell by default when Grid is called
    N_pix = 1
