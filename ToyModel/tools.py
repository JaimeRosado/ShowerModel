...
def show_projection(projection, profile, shower_size, axes, max_theta, X_mark):
    ...
    fig = plt.figure()
    ax1 = fig.add_subplot(121, projection='polar') #Plot in horizontal coordinates
    ax1.set_title('Horizontal coordinates alt/az', y=1.1)
    
    ax2 = fig.add_subplot(122, projection='polar') #Plot in FoV coordinates        
    ax2.set_title('FoV coordinates theta/phi', y=1.1)
    
    if shower_size: #Only available for shower and event
        #For the shower track, the point radius is proportional to the shower size      
    ...
    if axes:
        #Coordinates of the limits of a 2*pi solid angle around the telescope pointing direction in FoV projection
        ...
        alt_we = np.linspace(0., 180., 61) #Values greater than 90 degrees correspond to az_we = 180 degrees
        az_we = alt_hor #np.zeros(61)
        #In FoV projection
        theta_we, phi_we = telescope.altaz_to_thetaphi(alt_we, az_we)
        ax2.plot(np.radians(phi_we), theta_we, 'g--')
       
        #Coordinates of south-zenith-north arc in horiziontal coordinates system
        ...
        #Coordinates of the arc phi=0/180 (going through north point and the telescope pointing direction) in FoV coordinates system
        theta_phi0 = alt_we - 90.  #= np.linspace(-90., 90., 61)  #Negative values correspond to phi = 180 degrees
        ...
        theta_phi90 = theta_phi0 #= np.linspace(-90., 90., 61)  #Negative values correspond to phi = 270 degrees
        phi_phi90 = theta_2pi #=np.ones(61) * 90.
        #Horizontal coordinates system
        alt_phi90, az_phi90 = telescope.thetaphi_to_altaz(theta_phi90, phi_phi90)
        ax1.plot(np.radians(az_phi90), alt_phi90, 'g--')
    
    if X_mark!=None:
        #Absolute coordinates of the shower point at the input slanth depth X_mark
        #x_mark, y_mark, z_mark = telescope._abs_to_rel(*track.X_to_xyz(X_mark))
        x_mark, y_mark, z_mark = track.X_to_xyz(X_mark)
        r_mark, alt_mark, az_mark, theta_mark, phi_mark = telescope.spherical(x_mark, y_mark, z_mark)
    ...
    ax2.set_theta_zero_location('E', offset=telescope.phi_right)  #offset should be the north direction from right-hand direction
    ax2.set_theta_direction(-1)
    #theta is the name of the axial angle in the plot, not the coordinate theta !
    plt.tight_layout()
...
def show_geometry(obj, observatory, mode, x_min, x_max, y_min, y_max,
                  X_mark, shower_size, signal_size, tel_index, xy_proj, pointing):
    """Show a shower track together with the telescope positions of an observatory in an either 2D or 3D plot."""
    ...
    #Track points
    data_range = (x_min<track.x) & (track.x<x_max) & (y_min<track.y) & (track.y<y_max)
    x_track = np.array(track.x[data_range])
    y_track = np.array(track.y[data_range])
    z_track = np.array(track.z[data_range])
    
    #Telescope positions
    coords=[(telescope.x, telescope.y, telescope.z) for telescope in observatory]
    ...
    y_line = y_track.copy()
    z_line = z_track.copy()
    if z_tel_min<z_track.min():  #The observation level is lower than the lowest track point
        x_line = np.insert(x_line, 0, track.x0 + z_tel_min * track.ux / track.uz)
        y_line = np.insert(y_line, 0, track.y0 + z_tel_min * track.uy / track.uz)
        z_line = np.insert(z_line, 0, z_tel_min)
    if z_tel_max>z_track.max(): #The observation level is higher than the highest track point
    ...
    if signal_size: #Only available for event, when some signal is produced
        #For the telescope positions, the point radius is prportional to the signal
        signal_size = np.array([signal.Npe_total_sum for signal in obj.signals])
    ...
    if mode=='2d':  #2d projection
        ...
        ax.scatter(x_tel, y_tel, c='g', s=signal_size, marker='o') #Telescope positions in green
        ax.plot(x_line, y_line, 'r-') #Shower track line in red
        ax.scatter(x_track, y_track, c='r', s=shw_size, marker='o') #Shower track points in red
        ...
        if X_mark!=None:
            #Coordinates of the shower corresponding to the input slant depth X_mark
            x_mark, y_mark, z_mark = track.X_to_xyz(X_mark)
        ...
        ax.scatter(x_tel, y_tel, z_tel, c='g', s=signal_size, marker='o') #Telescope positions in green
        ax.plot(x_line, y_line, z_line, 'r-') #Shower track line in red
        if xy_proj:
            ax.plot(x_line, y_line, 'r--', zs=z_min, zdir='z') #xy projection of shower track line in red
        ax.scatter(x_track, y_track, z_track, c='r', s=shw_size, marker='o') #Shower track points in red
        
        if pointing:
            #Arrows pointing to the telescope axis direction
            pointing = [(telescope.ux, telescope.uy, telescope.uz) for telescope in observatory]
            ux, uy, uz = zip(*pointing)
            ax.quiver(x_tel, y_tel, z_tel, ux, uy, uz, length=0.4)
        
        if X_mark!=None:
            #Coordinates of the shower corresponding to the input slanth depth X_mark
            x_mark, y_mark, z_mark = track.X_to_xyz(X_mark)
...
def rotate(vx, vy, vz, rot_angle, x, y, z):
    """Rotate the vector (x,y,z) by an angle rot_angle (positive or negative) around the axis (vx,vy,vz), where v is a unit vector
    """
    ct = math.cos( math.radians(rot_angle) )
    st = math.sin( math.radians(rot_angle) )
    x_rot =      ( ct + vx * vx * (1.-ct) ) * x + ( vx * vy * (1.-ct) - vz * st ) * y + ( vx * vz * (1.-ct) + vy * st ) * z
    y_rot = ( vx * vy * (1.-ct) + vz * st ) * x +      ( ct + vy * vy * (1.-ct) ) * y + ( vy * vz * (1.-ct) - vx * st ) * z
    z_rot = ( vx * vz * (1.-ct) - vy * st ) * x + ( vy * vz * (1.-ct) + vx * st ) * y +      ( ct + vz * vz * (1.-ct) ) * z
...
where theta is defined from the z axis"""
    try:  #For r being a scalar (otherwise r==0 generates an exception)
        ...
        return 1.*theta #If z is also a scalar, the function returns a scalar. Otherwise, the output is an array           

    except: #For r being an array (or a Series)
        r = np.array(r)
        if not isinstance(z, (tuple, list, np.ndarray, pd.Series)): #For z being a scalar
            z = np.ones_like(r) * z           #An array of same lenght as r is generated
        else:
            z = np.array(z)
        z_r = np.zeros_like(r)
        z_r[r!=0] = z[r!=0] / r[r!=0]
...
where phi is defined from the x axis towards the y axis (anticlockwise)"""
    try:  #For x being a scalar (otherwise x==0 generates an exception)
        if x==0:
            ...
            if x<0:
                phi = phi + 180.
        return 1.*phi  #If y is also a scalar, the function returns a scalar. Otherwise, the output is an array
    
    except: #For x being an array (or a Series)
        x = np.array(x)
        if not isinstance(y, (tuple, list, np.ndarray, pd.Series)): #For r being a scalar
            y = np.ones_like(x) * y             #An array of same lenght as x is generated
        else:
            y = np.array(y)
