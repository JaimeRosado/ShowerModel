...
#### Constructor ####################################
def Track(theta=_theta, alt=None, az=_az, x0=_x0, y0=_y0, atmosphere=None, **kwargs):
    """Make a shower track discretization. 
    
Parameters
----------
theta : Zenith angle in degrees of the apparent position of the cosmic-ray source.
alt : Altitude in degrees of the apperent position of the cosmic-ray source.
...
**kwargs {h0, h_top, N_steps, model} : Options to construct the new Atmosphere object when atm==None.
...
Shower : Contructor of Shower object.
    ...
    #The columns of the output DataFrame includes, at each discretization step of are:
    #The coordinates (x,y,z) in km and the travel time t in us
    track = _Track( columns=['x', 'y', 'z', 't'] )
    track.atmosphere = atmosphere
    
    #The input parameters along with some geometric parameters are also included as atributes of the DataFrame  
    #The angles are stored in degrees
    if alt==None:
        alt = 90. - theta
    else:
        theta = 90. - alt
    ...
    #Coordinates of a unit vector perpendicular to u and parallel to horizontal xy plane
    track.vx = cos_az
    track.vy = -sin_az
    track.vz = 0.
    ...
    #Core position at ground level (z=0) and on the top of the atmosphere (z=zmax)
    track.x0 = x0
    track.y0 = y0
    ...
    #Total travel time in us, where t=0 corresponds to the moment when the shower enters the atmosphere
...
az : Azimuth angle (from north, clockwise) in degrees of the apparent position of the cosmic-ray source.
ux : East coordinate of a unit vector parallel to  the shower axis (upwards).
uy : North coordinate of a unit vector parallel to the shower axis (upwards).
uz : Vertical coordinate of a unit vector parallel to the shower axis (upwards).
vx : East coordinate of a unit vector perpendicular to shower axis and parallel to horizontal plane.
vy : North coordinate of a unit vector perpendicular to shower axis and parallel to horizontal plane.
...
Projection.show
    ...
    def show_projection(self, telescope, axes=True, max_theta=30., X_mark=None):
...
Projection.show
    ...
    def show_geometry2D(self, observatory, x_min=-1., x_max=1., y_min=-1, y_max=1., X_mark=None, tel_index=False):
...
AxesSubplot object.
        ...
        return show_geometry(self, observatory, '2d', x_min, x_max, y_min, y_max,
                             X_mark, False, False, tel_index, False, False)
       
        
    def show_geometry3D(self, observatory, x_min=-1., x_max=1., y_min=-1, y_max=1., X_mark=None, xy_proj=True, pointing=False):
...
Axes3DSubplot object.
        ...
        return show_geometry(self, observatory, '3d', x_min, x_max, y_min, y_max,
                             X_mark, False, False, False, xy_proj, pointing)
